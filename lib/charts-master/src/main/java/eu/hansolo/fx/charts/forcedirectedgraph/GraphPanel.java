    ...
    public void redraw() {
        ...
        for (GraphNode node : nodeEdgeModel.getNodes()) {
            ...
            if (null != name && !name.isBlank()) {
                double nodeX = xPositionRealToDrawn(node.getPosition().getX());
                double nodeY = yPositionRealToDrawn(node.getPosition().getY());
                double nodeRadius = (val * sizeFactor + minRadius) * calculateNodeAndEdgeScaleFactor();

                ctx.save();
                ctx.setFont(Font.font("System", 11));
                ctx.setTextAlign(TextAlignment.CENTER);
                ctx.setFill(Color.rgb(47, 51, 56));
                ctx.fillText(name, nodeX, nodeY + nodeRadius + 14);
                ctx.restore();
            }
        }
    }
